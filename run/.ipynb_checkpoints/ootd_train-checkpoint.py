...
def get_opt():
    ...
    parser.add_argument('--dataset_dir', type=str, default='/VTONDataset/VITON-HD/zalando-hd-resized/')
...
sys.path.append(r'/OOTDiffusion/ootd')

from pipelines_ootd.unet_vton_2d_condition import UNetVton2DConditionModel
from pipelines_ootd.unet_garm_2d_condition import UNetGarm2DConditionModel

VIT_PATH = "/OOTDiffusion/checkpoints/clip-vit-large-patch14"
VAE_PATH = "/OOTDiffusion/checkpoints/ootd"
UNET_PATH = "/OOTDiffusion/checkpoints/ootd/ootd_hd/checkpoint-36000/"
MODEL_PATH = "/OOTDiffusion/checkpoints/ootd"
scheduler_path = '/OOTDiffusion/checkpoints/ootd/scheduler/scheduler_ootd_config.json'
...
# params_to_optimize = list(unet_vton.parameters())
...
num_update_steps_per_epoch = math.ceil(len(train_dataloader.data_loader) / args.gradient_accumulation_steps)
...
# unet_garm,unet_vton, optimizer, train_dataloader, lr_scheduler, vae,image_encoder,text_encoder = accelerator.prepare(
#         unet_garm,unet_vton, optimizer, train_dataloader, lr_scheduler,vae,image_encoder,text_encoder
#     )
...
##########   torch.compile  ###########
# torch.compile(vae)
# torch.compile(unet_garm)
# torch.compile(unet_vton)
...
logger.info(f"  Num batches each epoch = {len(train_dataloader.data_loader)}")
...
for epoch in range(first_epoch, args.num_train_epochs):
    for step, batch in enumerate(train_dataloader.data_loader):
        with accelerator.accumulate(unet_vton),accelerator.accumulate(unet_garm):
        # with accelerator.accumulate(unet_garm):
            ...
            #     loss = F.mse_loss(noise_pred.half(), noise, reduction="mean")
            # else:
